...
def test_custom_model_created(monkeypatch):
    ...
    assert (
        ...
        == agent_infra.agent_langgraph_execution_environment.id
    )
    assert (
...
def test_agent_deployment_created_when_env(monkeypatch):
    ...
    agent_infra.pulumi.export.assert_any_call(
        "Agent Deployment ID " + agent_infra.agent_langgraph_resource_name,
        agent_infra.CustomModelDeployment.return_value.id,
    )
    agent_infra.pulumi.export.assert_any_call(
        "Agent Deployment Chat Endpoint " + agent_infra.agent_langgraph_resource_name,
